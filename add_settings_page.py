...
# Add _load_settings and _save_settings at the end of the file
SETTINGS_HELPERS = """
...
async def _save_sys_settings(settings: Settings, data: dict):
    ...
    try:
        ...
        await db.dispose()
"""
if "_load_sys_settings" not in content:
    content += SETTINGS_HELPERS

# Add settings_page before accounts_page
accounts_idx = content.find("        def accounts_page(self) -> QWidget:")
if accounts_idx == -1:
    print("Error finding accounts_page")
    sys.exit(1)

SETTINGS_PAGE_CODE = """        def settings_page(self) -> QWidget:
            ...
            def make_group(title_text, icon_text, color):
                g = QFrame(); g.setStyleSheet(f"QFrame {{ background: white; border: 1px solid #e2e8f0; border-radius: 8px; }}")
...
"""
if "def settings_page(" not in content:
    content = content[:accounts_idx] + SETTINGS_PAGE_CODE + content[accounts_idx:]

with open("src/pmql/ui/app.py", "w", encoding="utf-8") as f:
    f.write(content)
print("Added settings_page logic to app.py")
