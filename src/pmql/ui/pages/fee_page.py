...
class FeePageMixin:
    def fee_page(self) -> QWidget:
            ...
            # Grid of rules
            try:
                rules = asyncio.run(_fee_rules(self.settings))
            ...
            def do_calc():
                try:
                    ...
                    all_rules = asyncio.run(_fee_rules(self.settings))
    ...
    def add_fee_rule(self) -> None:
            ...
            def save_rule() -> None:
                try:
                    asyncio.run(_create_fee_rule(self.settings, name.text(), vehicle.currentData(), int(block.text()), int(price.text()), int(free.text()), int(surcharge.text()), int(maximum.text()) if maximum.text() else None)); dialog.accept(); self.reload_page("fees")
    ...
    def edit_fee_rule(self, rule) -> None:
            ...
            def save_rule() -> None:
                try: asyncio.run(_update_fee_rule(self.settings, rule.id, name.text(), vehicle.currentData(), int(block.text()), int(price.text()), int(free.text()), int(surcharge.text()), int(maximum.text()) if maximum.text() else None)); dialog.accept(); self.reload_page("fees")
                except Exception as exc: show_toast(dialog, str(exc), "error")
            save.clicked.connect(save_rule); dialog.exec()
    ...
    def delete_fee_rule(self, rule) -> None:
            ...
            try: asyncio.run(_delete_fee_rule(self.settings, rule.id)); self.reload_page("fees")
    ...
    def show_fee_history(self):
            ...
            try:
                snap_rules = asyncio.run(_fee_rules(self.settings))
                snap_names = asyncio.run(_vehicle_name_map(self.settings))
