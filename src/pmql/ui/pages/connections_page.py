...
class ConnectionsPageMixin:
    def connections_page(self) -> QWidget:
            page, box = self.page()
            
            # Title header matching v1 look slightly
            title_h = QHBoxLayout()
            import qtawesome as qta
            ico = label("")
            ico.setPixmap(qta.icon("fa5s.plug", color="#f59e0b").pixmap(20, 20))
            title_h.addWidget(ico)
            ...
            def section_box(icon, title_text, color):
                f = QFrame(); f.setStyleSheet("QFrame { background: white; border: 1px solid #e2e8f0; border-radius: 8px; }")
            ...
            note = label("ℹ Chỉ hiển thị thiết bị đang kết nối TCP thực tế vào cổng 9001")
            ...
            # Wizard Header
            wizard_frame = QFrame()
            wizard_frame.setStyleSheet("background: #3b82f6; border-radius: 8px;")
            wiz_l = QHBoxLayout(wizard_frame); wiz_l.setContentsMargins(20, 20, 20, 20)
            wiz_text = QVBoxLayout()
            wiz_t1 = label("⚡ Wizard Kết nối thiết bị phần cứng", bold=True)
            wiz_t1.setStyleSheet("color: white; font-size: 18px;")
            wiz_t2 = label("Hướng dẫn từng bước kết nối đầu đọc thẻ RFID, camera ANPR, vân tay và barrier với phần mềm PMQL.")
            wiz_t2.setStyleSheet("color: #bfdbfe; font-size: 13px;")
            wiz_text.addWidget(wiz_t1); wiz_text.addWidget(wiz_t2)
            wiz_l.addLayout(wiz_text); wiz_l.addStretch()
            wiz_icon = label(""); wiz_icon.setPixmap(qta.icon("fa5s.microchip", color="#bfdbfe").pixmap(40, 40))
            wiz_l.addWidget(wiz_icon)
            left_col.addWidget(wizard_frame)
            ...
            def make_device_card(key, icon, name, sub, color, protos):
                ...
                def on_click(_, k=key, c=color, ps=protos, nm=name):
                    ...
                    for ck, cf in device_card_refs.items():
                        if ck == k:
                            cf.setStyleSheet(f"QFrame {{ background: {c}10; border: 1px solid {c}; border-radius: 10px; }}")
                        else:
            ...
            test_btn = icon_btn("fa5s.bolt", "Test kết nối", _BTN_PLAIN_STYLE)
            ...
            save_btn = icon_btn("fa5s.save", "Lưu thiết bị", _BTN_PLAIN_STYLE)
            ...
            # ── Resources section ────────────────────────────────────────
            res_h = QHBoxLayout(); res_h.setSpacing(12)
            for icon_txt, title_r, sub_r, color_r in [
                ("fa5s.file-code","Driver Python TCP","Tài liệu PDF - client TCP","#3b82f6"),
                ("fa5s.microchip","Arduino / ESP32","Tài liệu PDF - RFID TCP","#22c55e"),
                ("fa5b.raspberry-pi","Raspberry Pi","Tài liệu PDF - Pi + Camera","#a855f7"),
                ("fa5s.file-pdf","Tài liệu giao thức","PDF - TCP Protocol full","#f97316"),
            ]:
                rc = QFrame(); rc.setStyleSheet("QFrame{background:white; border: 1px solid #e2e8f0; border-radius:8px;}")
                rv = QVBoxLayout(rc); rv.setContentsMargins(12,12,12,12); rv.setSpacing(6)
                
                h_top = QHBoxLayout()
                ri = label(""); ri.setPixmap(qta.icon(icon_txt, color=color_r).pixmap(20, 20))
                h_top.addWidget(ri)
                
                v_txt = QVBoxLayout(); v_txt.setSpacing(0)
                v_txt.addWidget(label(title_r, bold=True))
                v_txt.addWidget(label(sub_r, "muted"))
                h_top.addLayout(v_txt); h_top.addStretch()
                rv.addLayout(h_top)
                
                rb = icon_btn("fa5s.download", "Xuất PDF", _BTN_PLAIN_STYLE)
                rb.setStyleSheet(f"background: white; color: {color_r}; border: 1px solid {color_r}; border-radius: 4px; padding: 4px; font-weight: 600; font-size: 11px;")
                rv.addWidget(rb); res_h.addWidget(rc)
            left_col.addLayout(res_h)
            left_col.addStretch()
